 ...
 * @param attr_type the type of the fetched attribute
 ...
 */
static inline int
rbh_backend_get_attribute(struct rbh_backend *backend, const char *attr_type,
                          void *arg, struct rbh_value_pair *data)
{
    ...
    return backend->ops->get_attribute(backend, attr_type, arg, data);
}
