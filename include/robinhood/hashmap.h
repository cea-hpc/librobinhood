 ...
 *     +----------------+----------------+
 *
 * And to query the hashmap for a give key, use rbh_hashmap_get().
 *
 ...
 *
 * To remove a vlue from a hashmap, use rbh_hashmap_pop(). A pointer to the
 ...
 *
 * @error EINVAL    \p count must not be zero
...
/**
 * Associate a key to value in a hashmap
 *
