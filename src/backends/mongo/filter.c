                       ...
                       const struct rbh_value *value, bool negate)
{
    ...
    case RBH_FOP_REGEX:
        ...
        break;
    case RBH_FOP_EXISTS:
        /* The exists operator doesn't have any negated counterpart, so the
         * "not exists" operator is just the "exists" operator with the boolean
         * value reversed.
         */
        assert(value->type == RBH_VT_BOOLEAN);
        tmp_value.type = value->type;
        tmp_value.boolean = value->boolean ^ negate;
        value = &tmp_value;
        break;
    default:
        break;
