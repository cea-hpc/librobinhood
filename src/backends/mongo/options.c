        ...
        )
{
     ...
     * Or:
     *     {field-1: true, field-2: true}
     *
     * That is because {field-0: true} is interpreted as "nothing other than
     * `field-0`", whereas {field-0: false} is interpreted as "everything but
     * `field-0`", and those two interpretation don't mix well with one another
     *
    ...
    if (fsentry_mask == 0)
        ...
        return bson_append_document_begin(bson, key, key_length, &document)
            ...
            && BSON_APPEND_BOOL(&document, MFF_PARENT_ID, false)
