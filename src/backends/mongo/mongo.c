...
static size_t
ringr_latest_reader(struct rbh_ringr *ringr[])
{
    size_t size[2];
...
filter_child_fsentries(struct rbh_backend *backend,
                       ...
                       enum ringr_reader_type reader_type,
                       const struct rbh_filter *filter,
                       const struct rbh_filter_options *options)
{
    struct rbh_mut_iterator *iterator;
    ...
    value_list = rbh_ringr_peek(values[reader_type], &count);
    if (count == 0) {
        errno = ENODATA;
        return NULL;
    }
    ...
    rbh_ringr_ack(values[reader_type], count * sizeof(*value_list));
    rbh_ringr_ack(ids[reader_type], id_sum);
...
{
    return filter_child_fsentries(iter->backend, iter->values, iter->ids,
                                  RRT_FSENTRIES, iter->filter, &iter->options);
}

static int
__append_id(struct branch_iterator *iter, int old_errno,
            struct rbh_mut_iterator **retval)
{
    ...
    if (value->binary.data == NULL) {
        ...
        case ENOBUFS: /* The ring is full */
            ...
            if (ringr_latest_reader(iter->ids) == RRT_DIRECTORIES) {
                ...
                return __append_id(iter, old_errno, retval);
            }
...
static int
__append_value(struct branch_iterator *iter, int old_errno,
               struct rbh_mut_iterator **retval)
{
    struct rbh_value *value = &iter->value;
    ...
    if (rbh_ringr_push(iter->values[RRT_ANY], (char *)value, sizeof(*value))
        ...
        case ENOBUFS: /* The ring is full */
            ...
            if (ringr_latest_reader(iter->ids) == RRT_DIRECTORIES) {
                ...
                return __append_value(iter, old_errno, retval);
            }
            errno = old_errno;
            *retval = __branch_next_directory(iter);
            return 1;
...
record_id:
        ...
        if (__append_id(iter, old_errno, &retval))
            return retval;
...
record_rbh_value:
        ...
        if (__append_value(iter, old_errno, &retval))
            return retval;
