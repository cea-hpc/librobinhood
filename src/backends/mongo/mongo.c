...
mongo_iter_next(void *iterator)
{
     ...
     * as DONE, resulting in a MONGOC_ERROR_CURSOR_INVALID_CURSOR
     * obtained with mongoc_cursor_error, but which corresponds to calls
     * to mongoc_cursor_next on completed or failed states. Since mongoc
     * doesn't allow us to differentiate the two, we check beforehand that
