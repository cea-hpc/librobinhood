...
fsentry_from_ftsent(FTSENT *ftsent, int statx_sync_type, size_t prefix_len,
                    ssize_t (*ns_xattrs_callback)(int, struct rbh_value_pair *,
                                                  size_t *,
                                                  struct rbh_sstack *))
{
    ...
    if (ns_values == NULL) {
        ...
        ns_values = rbh_sstack_new(sizeof(struct rbh_value) * ns_pairs_count);
        if (ns_values == NULL)
            return NULL;
    }
    ...
    if (ns_xattrs_callback != NULL) {
        ns_count = ns_xattrs_callback(fd, &ns_pairs[ns_xattrs.count],
                                      &ns_pairs_count, ns_values);
        if (ns_count == -1) {
            save_errno = errno;
            goto out_clear_sstacks;
        }
