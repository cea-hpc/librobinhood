...
static int
attr2uint(char *attrs, char *attr, uint64_t *value)
{
    int save_errno;
    char *found;
    int rc = 0;
     ...
     * So to parse a specific attribute, we have to find in the string, go
...
static int
get_xattrs(char *attrs, struct rbh_value_pair **_pairs,
           struct rbh_sstack *values)
{
