...
START_TEST(rra_multi_loop)
{
    ...
    rbh_ringr_push(ringr, buffer, page_size * 3 / 4);
    rbh_ringr_ack(ringr, page_size / 2);
    rbh_ringr_ack(duplicate, page_size / 2);

    rbh_ringr_push(ringr, buffer, page_size / 2);
    rbh_ringr_ack(ringr, page_size / 2 + 1);
    rbh_ringr_ack(duplicate, page_size / 2 - 1);

    rbh_ringr_push(ringr, buffer, page_size * 3 / 4);
    rbh_ringr_ack(ringr, page_size - 1);
    rbh_ringr_ack(duplicate, page_size + 1);
